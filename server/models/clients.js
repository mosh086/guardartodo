...
model.getAll = function(callback) {
  if (connection) {
    var sql = `SELECT CONCAT_WS(' ',firstName,lastName) as fullName, c.* FROM guardartodo.client c`;
...
model.insert = function(clientData,callback)
{
    if (connection)
    {
        connection.query('INSERT INTO client SET ?', clientData, function(error, result)
        {
            if(error)
            {
