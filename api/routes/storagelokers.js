...
function getAll (done) {
  db.get().query('SELECT * FROM storageloker', function(err, rows) {
    if(err) throw err;
    done(rows);
  });
}
...
function getById (id,done) {
  db.get().query('SELECT * FROM storageloker WHERE storagelokerId = ?', id, function(err, row) {
    if(err) throw err;
    done(row);
  });
}
...
function update (id, data, done) {
  db.get().query('UPDATE storageloker SET ? WHERE storagelokerId = ?', [data, id], function(err, result) {
    if(err) throw err;
    done(result);
  });
}
...
function remove (id, done) {
  db.get().query('DELETE FROM storageloker WHERE storagelokerId = ?', id, function(err, result) {
    if(err) throw err;
    done(result);
  });
}
